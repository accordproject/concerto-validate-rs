...
fn main() {
    ...
    let exit_code = match cli.command {
        Commands::Validate { input, fail_early } => {
            handle_validate_command(input, fail_early)
        }
    };
...
fn handle_validate_command(input_files: Vec<PathBuf>, fail_early: bool) -> i32 {
    if input_files.is_empty() {
        eprintln!("Error: No input files specified. Use --input to specify JSON files to validate.");
        return 1;
    }
